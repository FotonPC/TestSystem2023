...
class TNABaseUG(ttk.LabelFrame):
    def __init__(self, master=None, equals=None):
        ...
        self.style.configure('std.TButton', background='gray', font='Arial 10')
...
class MAPQuiZ(ttk.Frame):
    def __init__(self, master=None,  packq=None, root=None):
        ...
        for val in self.values:
            self.buttons.append(ttk.Button(self.mapframe, text='', style='std.TButton', width=1,
                                           command=lambda event=None, index=i: self.select(ind=index)))
