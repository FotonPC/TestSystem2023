...
class App(ttkthemes.ThemedTk):
    ...
    def init_test_tab(self):
        ...
        self.test_tab_butt_add_test = ttk.Button(self.test_tab_labelframe_operate_tests, text='Добавить тест +',
                                                 command=self.add_test)
        self.test_tab_butt_add_test.place(relx=0, rely=0, anchor='nw', relwidth=0.33, relheight=1)
        self.test_tab_butt_launch_test = ttk.Button(self.test_tab_labelframe_operate_tests, text='Запустить тест',
                                                    command=self.launch_test)
        self.test_tab_butt_launch_test.place(relx=0.33, rely=0, relheight=1, relwidth=0.34, anchor='nw')
        self.test_tab_butt_del_test = ttk.Button(self.test_tab_labelframe_operate_tests, text='Удалить тест -',
    ...
    def add_test(self, event=None):
        filename = filedialog.askopenfilename(
            filetypes=(('FTS23 Карта-Тест V2 .fts23v2', '*.fts23v2'), ('FTS23 Тест-Соответствие', '*.fts23m')))
